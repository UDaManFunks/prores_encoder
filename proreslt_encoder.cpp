...
const uint8_t ProResLTEncoder::s_UUID[] = { 0x21, 0x42, 0xe8, 0x41, 0xd8, 0xe4, 0x41, 0x4b, 0x87, 0x9e, 0xa4, 0x80, 0xfc, 0x90, 0xda, 0xb7 };

const ProfileMap ProResLTEncoder::s_ProfileMap[1] = { {"1", 'apcs', AV_PIX_FMT_YUV422P10LE , "ProRes 422 (LT)"} };

StatusCode ProResLTEncoder::s_GetEncoderSettings(HostPropertyCollectionRef* p_pValues, HostListRef* p_pSettingsList)
{
...
StatusCode ProResLTEncoder::s_RegisterCodecs(HostListRef* p_pList)
{
	...
	codecInfo.SetProperty(pIOPropUUID, propTypeUInt8, ProResLTEncoder::s_UUID, 16);
	...
	g_Log(logLevelInfo, "ProResLT Plugin :: s_RegisterCodecs :: fourCC = %d", vFourCC);
...
StatusCode ProResLTEncoder::DoInit(HostPropertyCollectionRef* p_pProps)
{

	uint32_t vFourCC = 0;
	p_pProps->GetUINT32(pIOPropFourCC, vFourCC);

	g_Log(logLevelInfo, "ProResLT Plugin :: DoInit :: fourCC = %d", vFourCC);

	return errNone;
}

StatusCode ProResLTEncoder::DoOpen(HostBufferRef* p_pBuff)
{
...
StatusCode ProResLTEncoder::DoProcess(HostBufferRef* p_pBuff)
{
	return m_pWorker->EncodeFrame(p_pBuff, m_pCallback);
}

void ProResLTEncoder::DoFlush()
{

	g_Log(logLevelInfo, "ProResLT Plugin :: DoFlush");
